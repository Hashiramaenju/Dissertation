...
def main():
    ...
    with col_left:
        ...
        # Search box
        search_term = st.text_input("🔍 Search symptoms...", "", key="symptom_search", label_visibility="collapsed", placeholder="Search symptoms...")

        # Filter symptoms based on search
        filtered_symptoms = sorted(SYMPTOMS)
        if search_term:
            filtered_symptoms = [s for s in sorted(SYMPTOMS) if search_term.lower() in s.replace('_', ' ')]

        # Render checkboxes in a 3-column grid (matches the dashboard screenshot)
        st.markdown("""
        <style>
        ...
        st.markdown('<div style="max-height: 400px; overflow-y: auto; padding-right: 4px;">', unsafe_allow_html=True)

        cols = st.columns(3)
        newly_selected = []
        for i, symptom in enumerate(filtered_symptoms):
            is_checked = symptom in st.session_state.selected_symptoms
            checked = cols[i % 3].checkbox(
                format_symptom_name(symptom),
                value=is_checked,
                key=f"cb_{symptom}"
            )
            if checked:
        ...
        st.markdown(f"<p style='color: #94a3b8; font-size: 0.9rem; margin-top: 8px;'>📊 {len(st.session_state.selected_symptoms)} symptoms selected</p>", unsafe_allow_html=True)
    ...
    st.bar_chart(df_perf.set_index('Metric'), height=200, color='#6366f1')
    ...
    st.bar_chart(df_disease.set_index('Category'), height=200, color='#10b981')
