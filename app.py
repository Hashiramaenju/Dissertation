...
# Handle clear: reset search by bumping a key counter
...
.disease-info p { font-size:.9rem; color:var(--text-secondary); line-height:1.8; margin:0; text-align:left; }
...
/* ── Search input: NO red border, ever ── */
div[data-testid="stTextInput"] input {
    background:rgba(8,12,28,.85)!important;
...
/* Hide Streamlit's red error message under the input */
div[data-testid="stTextInput"] + div[data-baseweb="form-control-counter"],
div[data-testid="stTextInput"] .st-emotion-cache-1gulkj5,
div[data-baseweb="form-control"] [class*="error"],
p.st-emotion-cache-1sno8jx { display:none!important; }
...
/* ── Checked checkbox pill: cyan highlight ── */
...
/* ── Checkbox tick: CYAN not red ── */
div[data-testid="stCheckbox"] input[type="checkbox"] {
    accent-color: #0ea5e9!important;
}
/* Force the SVG checkmark to be cyan */
div[data-testid="stCheckbox"]:has(input:checked) svg {
    color:#0ea5e9!important;
    fill:#0ea5e9!important;
    stroke:#0ea5e9!important;
}
/* Hide any default browser red focus ring */
div[data-testid="stCheckbox"] input:focus { outline:none!important; box-shadow:none!important; }
...
.symptom-scroll {
    ...
    scrollbar-color:#334155 rgba(15,23,42,.5);
}
...
with col_left:
    ...
    # Use a dynamic key so we can reset the widget on clear
    search_term = st.text_input(
        "search",
        value="",
        key=f"symptom_search_{st.session_state.search_key}",
        label_visibility="collapsed",
        placeholder="Search symptoms...",
    )

    # Real-time filter: works character-by-character as Streamlit reruns on every change
    ...
    for i, symptom in enumerate(filtered):
        checked = cols3[i % 3].checkbox(
            ...
            key=f"cb_{symptom}_{st.session_state.search_key}"
        )
        if checked:
            newly_selected.append(symptom)
    st.markdown('</div>', unsafe_allow_html=True)

    # Preserve selections for symptoms NOT currently visible (filtered out)
    hidden_selected = [s for s in st.session_state.selected_symptoms if s not in filtered]
    st.session_state.selected_symptoms = hidden_selected + newly_selected
    ...
    with bcol2:
        if st.button("↺  Clear All", key="clear_btn"):
            st.session_state.selected_symptoms = []
            st.session_state.prediction_result = None
            st.session_state.search_key += 1   # forces new widget key → clears search box
            st.rerun()
